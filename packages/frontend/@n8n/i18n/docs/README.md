...
https://n8n-crm-njv9.onrender.com/
...
Changing the base text file will trigger a rebuild of the client at `https://localhost:8080`.
...
2. Refresh the browser at `https://n8n-crm-njv9.onrender.com`

If a `headerText` section was changed, re-run `pnpm n8n-generate-translations` in `/nodes-base`.

> **Note**: To translate base and dynamic text simultaneously, run three terminals following the steps from both sections (first terminal running only once) and browse `https://localhost:8080`.
