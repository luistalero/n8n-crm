...
describe('Test DiscordV2, message => sendAndWait', () => {
	...
	it('should send message and put execution to wait', async () => {
		...
		mockExecuteFunctions.evaluateExpression.mockReturnValueOnce('https://localhost/waiting-webhook');
		mockExecuteFunctions.evaluateExpression.mockReturnValueOnce('nodeID');

		mockExecuteFunctions.getSignedResumeUrl.mockReturnValue(
			'https://localhost/waiting-webhook/nodeID?approved=true&token=abc',
		);
						...
						components: [
							{
								...
								style: 5,
								type: 2,
								url: 'https://localhost/waiting-webhook/nodeID?approved=true&token=abc',
							},
						],
						type: 1,
					},
